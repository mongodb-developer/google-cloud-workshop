...
1. Since we have made changes to the app by adding these rules, we need to deploy those changes. This **Review Draft & Deploy** step is on my default, to protect us from making changes unintentionally.
...
1. We now want to create a schema for our **comments** collection. This time, we’ll write the schema instead of generating it. Click **Define a Schema**.

1. Paste the following JSON schema into the box.
...
1. We now need to define a relationship between our two collections so that the comment document understands cakeId. So, click **Add a relationship** and set it to the following before clicking **Add**:
    - Parent Field: **cakeId - ObjectId**
    - Linked Database: **Bakery**
    - Linked Collection: **cakes**
    - Linked Field: **_id**

1. At this point, we need to click **Review Draft & Deploy** at the top of the browser window, to deploy both our new schemas for our application.
